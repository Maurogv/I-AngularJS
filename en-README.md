...
As I wanted additional features, with **Javascript** I built the index, the notes, the list of domain names of each external link, a list is for photos only. Text mentions the sources, so it adds the reference, a hypertext number, and in the notes goes back up. Furthermore, my code before the domain name's link adds an icon, 16x16, if its domain is present in a library of a json file, or otherwise a standard symbol. 
