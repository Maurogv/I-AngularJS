...
var baseJSUrlHTML5 = 'http://static2.video.corriereobjects.it/widget/js/'
...
var allowedDomains="corriere.it,www.corriere.it,www2.corriere.it,social.corriere.it,cinema-tv.corriere.it,video.corriere.it,corrieredelveneto.corriere.it,corrieredelmezzogiorno.corriere.it,roma.corriere.it,milano.corriere.it,corrieredibologna.corriere.it,corrierefiorentino.corriere.it,cucina.corriere.it,motori.corriere.it,olimpiadi.corriere.it,auladigitale.rcs.it,videocrossbe.corriere.it";
...
var detectUserAgent =
{
	initialize: function()
	{
		...
		if(	(UA.match(/Chrome/i) != null || UA.match(/Firefox/i) != null) && PolymediaWidgetGetSwfVer() < 0)
		{
			device_event = "touchstart";
		}
	}
...
{
	var UA = navigator.userAgent.toLowerCase();
	if(UA.indexOf("windows mobile")!=-1){return true;}
	if(UA.indexOf("windows phone os")!=-1){return true;}
	if(UA.indexOf("windows phone")!=-1){return true;}
...
document.write('<script type="text/javascript" src="https://secure-it.imrworldwide.com/novms/js/2/ggcmb370.js"></scr' + 'ipt>'); 
...
{
	var link = basePolymediaShowUrl + relVideoPath + "video_" + videoId + ".rss";
	if(window.location.hostname.indexOf("auladigitale.rcs.it") > -1)
	{
		link = "http://www.auladigitale.rcs.it/widget/" + relVideoPath + "video_" + videoId + ".rss";
	}
	return link;
}

function synchronizePlaylist(videoId, playerId, channel, videoLink)
{
	videoCounter++;
	try 
	{
		videoGA (channel, videoCounter.toString());
	} catch (e) {}
	...
	try 
	{
		switch (playerId) 
		{
			case "1": LoadPlayerText1(videoId,playerId, videoLink); break;
			case "2": LoadPlayerText2(videoId,playerId, videoLink); break;
			case "3": LoadPlayerText3(videoId,playerId, videoLink); break;
			case "4": LoadPlayerText4(videoId,playerId, videoLink); break;
			case "5": LoadPlayerText5(videoId,playerId, videoLink); break;
			case "6": LoadPlayerText6(videoId,playerId, videoLink); break;
			case "7": LoadPlayerText7(videoId,playerId, videoLink); break;
		}
	} catch (e) {}
}
...
function CTVPlayer(configId, mainPlaylistId, mainPlaylistOrder, mainVideoId,  autoplay, showLogo, urlLogo, context, advChannel, playerId, writeOnDocument, popout, startAudioEnabled, pWidth, pHeigth, startAdvAudioEnabled)
{
	this.idWidget = 'PolymediaCorriere'+(countWidget++);

	if(window.location.hostname.indexOf("auladigitale.rcs.it") > -1)
	{
		basePolymediaShowUrl = "http://www.auladigitale.rcs.it/widget/";
	}
	...
	switch (configId)
	{
		...
		case 13: case "13": width = 398; height = 223; configId="4"; configAdvLabel = "embed"; configNielsenLabel = "embed"; break; // new Embed
		case 14: case "14": width = 854; height = 509; configId="1"; configAdvLabel = "tv"; configNielsenLabel = "tv"; break; // new Embed
		case 15: case "15": width = 636; height = 388; configId="2"; break; // player 636 uguale a player 400/620 article
		case 16: case "16": width = 621; height = 380; configId="1"; configAdvLabel = "tv"; configNielsenLabel = "tv"; break; // Aula Digitale
	}
	
	/*if(document.cookie.indexOf("__ric") == "-1"){
		autoplay = "false";
	}*/
		
	
	if(document.cookie.indexOf("cpmt_xa=5334") == "-1"){
		if(document.cookie.indexOf("__ric=") == "-1"){
			$.getScript("http://static2.advtools.rcsobjects.it/cpmt/corriere_ct.js")
			autoplay = "false";
		}	
	}
	...
	var logoUrl;
	if (urlLogo != null && urlLogo != "null" && (urlLogo.indexOf('.png')>-1 || urlLogo.indexOf('.jpg')>-1))
	{
		logoUrl = urlLogo;
	}
	else
	{
		logoUrl = basePolymediaShowUrl + relImgPath + "CTV_logo_white.png";
	}
	
	detectUserAgent.initialize();
	...
	if(device_event == "touchstart")
	{
		...
		if (mainVideoId != null &&  mainVideoId != "null" && mainVideoId != "")
		{
			...
			{
				if(mainVideoId.indexOf('http://static2.video.corriereobjects.it')>-1)
				{
					mainVideoId = mainVideoId.replace('http://static2.video.corriereobjects.it','http://video.corriere.it')
				}
				loadUrl = mainVideoId;
			}
		...
		{
			createPlayerHtml5(this.idWidget, loadUrl, width, height, autoplay, configAdvLabel, configNielsenLabel, advChannel, configId, logoUrl, context, startAdvAudioEnabled);
		}
		
		this.playVideo = function (videoId, autoPlay, doNotCheckID) {
			if(doNotCheckID == null)
			{
				doNotCheckID = false;
			}
			try
			{
				plctvjs.configholder[this.idWidget].html5loadVideo(videoId, getVideoUrl(videoId), autoPlay, doNotCheckID);
			}
			catch(e)
			{ 
				try
				{
					html5loadVideo(videoId, getVideoUrl(videoId), autoPlay, doNotCheckID);
				}
				catch(e){};
			}
		};
		...
		{
			try {
				return plctvjs.configholder[this.idWidget].getEmbedCode(width,height);
			} catch (e) {
				return "";
			}
		};
	...
	if (mainVideoId != null && mainVideoId != "null" && mainVideoId != '')
		{
			if(mainVideoId.indexOf("http") != -1)
			{
				var tempId = mainVideoId.split("/")[7];
				tempId = tempId.substring(0, tempId.indexOf("."));
				tempId = tempId.substring(tempId.indexOf("_")+1, tempId.lenght);
			}
			else
				var tempId = mainVideoId;
			if(!usepbURL)
			{
				flashVars += "&videoId=" + tempId; 
				flashVars += "&videoUrl=" + getVideoUrl(tempId);
			}
			else
			{
				flashVars += "&videoId=" + mainVideoId;
				flashVars += "&videoUrl=" + mainVideoId;
			}
		}
		...
		var isIEver  = (navigator.appVersion.indexOf("MSIE") != -1) ? true : false;
		if (isIEver) flashVars += "&IEbrowser=" + isIEver;
		...
		if (startAdvAudioEnabled != null && startAdvAudioEnabled != "null")
		{
			if(startAdvAudioEnabled == "true" || startAdvAudioEnabled == true)
			{
				flashVars += "&startAdvAudioEnabled=true";
			}else
			{
				flashVars += "&startAdvAudioEnabled=false";
			}
	...
	//	flashVars += "&relatedText="+"PROVA CAMBIO";
		...
		if (writeOnDocument != null && (writeOnDocument == false || writeOnDocument == "false")) {
			this.creationString = createWidget(this.idWidget, basePolymediaShowUrl + relSwfPath + 'PolymediaCorriere?v=20150206171137', flashVars, width, height, "string");
	//		this.creationString = createWidget(this.idWidget, basePolymediaShowUrl + relSwfPath + 'PolymediaCorriere', flashVars, width, height, "string");
			...
			createWidget(this.idWidget, basePolymediaShowUrl + relSwfPath + 'PolymediaCorriere?v=20150206171137', flashVars, width, height);
	//		createWidget(this.idWidget, basePolymediaShowUrl + relSwfPath + 'PolymediaCorriere', flashVars, width, height);
		...
		this.playVideo = function (videoId, autoPlay, doNotCheckID) 
		{
			...
			{
				_pl.loadVideo(videoId, getVideoUrl(videoId), autoPlay, doNotCheckID);
			}
			return false
		};
...
{
	var channel = FEED_CHANNEL[feed];
	if (feed == "MammaImperfetta2"){
		var umichannel = "UM2";
		return umichannel;
	}
	
	if (feed == "MammaImperfetta"){
		var umichannel = "UM1";
		return umichannel;
	}
	if (channel) 
...
//#############################################
function createPlayerHtml5(idWidget, loadVideoUrl, width, height, autoplay, configAdvLabel, configNielsenLabel, advChannel, pid, logoUrl, context, startAdvAudioEnabled)
{
	var out = "";
		out = "<div id='mainPlHTM"+idWidget+"' class='html5videoContainer'><video id='videoPlHTM"+idWidget+"' class='html5videoTag' poster controls='false' webkit-playsinline/></video><div id='advMessagePlHtml"+idWidget+"' class='html5AdvBar'><div class='html5AdvBarImg'></div></div><div id='watermarkPlHTM"+idWidget+"' class='html5watermarkContainer'></div><div id='advPlayPlHTM"+idWidget+"' class='html5PlayAdvContainer'><div id='adv_play_img' class='html5PlayAdvPlay' ></div></div></div>"
		document.write(out);
		
	try
	{
		$("#mainPlHTM"+idWidget).width(width);
		$("#mainPlHTM"+idWidget).height(height);
		$("#videoPlHTM"+idWidget).width(width);
		$("#videoPlHTM"+idWidget).height(height);
		$("#advMessagePlHtml"+idWidget).width(width);
		$("#advMessagePlHtml"+idWidget).hide();
		$("#advPlayPlHTM"+idWidget).width(width);
		$("#advPlayPlHTM"+idWidget).height(height);
		$("#advPlayPlHTM"+idWidget).hide();
		$("#watermarkPlHTM"+idWidget).hide();
		
		initMobilePlayer(idWidget, loadVideoUrl, autoplay, configAdvLabel, configNielsenLabel, advChannel, pid, logoUrl, width, height, context, startAdvAudioEnabled);
	}	
	catch (e)
	{
		$.getScript( baseJSUrlHTML5 + "polymediaPlayerMobileCorriere.js", function() {
			initMobilePlayer(idWidget, loadVideoUrl, autoplay, configAdvLabel, configNielsenLabel, advChannel, pid, logoUrl, width, height, context, startAdvAudioEnabled);
		});
	}	
}

function initMobilePlayer(idWidget, loadVideoUrl, autoplay, configAdvLabel, configNielsenLabel, advChannel, pid, logoUrl, width, height, context, startAdvAudioEnabled)
{
	window.plctvjs = plctvjs;
	window.CTVPlayerMobile = plctvjs.CTVPlayer;
	var p = new CTVPlayerMobile(idWidget, loadVideoUrl, autoplay, configAdvLabel, configNielsenLabel, advChannel, pid, logoUrl, window, width, height, context, startAdvAudioEnabled);
}
...
function createWidget(idWidget,src,params,width,height,outputFormat)
{
	...
	if (hasProductInstall && !hasRequestedVersion ) {
	   ...
	   var MMPlayerType = (isIEver == true) ? "ActiveX" : "PlugIn";
